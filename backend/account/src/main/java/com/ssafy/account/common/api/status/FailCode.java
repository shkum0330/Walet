...
public enum FailCode {
    ...
    NO_TRANSFER_ACCOUNT(BAD_REQUEST, "양도할 사람의 계좌가 존재하지 않습니다."),
    NO_TRANSFEREE_ACCOUNT(BAD_REQUEST, "양도받을 사람의 계좌가 존재하지 않습니다."),
    ...
    INCORRECT_PET_ACCOUNT_INFO(BAD_REQUEST, "반려동물의 이름 또는 계좌번호가 잘못 입력되었습니다."),
    NO_PET_ACCOUNT_WITH_AUTH_INFO(BAD_REQUEST, "입력한 인증정보에 해당하는 펫 계좌가 없거나 해당 계좌는 현재 거래가 불가능합니다."),
    NO_ACCOUNT_WITH_ACCOUNT_NUMBER(BAD_REQUEST, "계좌번호가 잘못 입력되었거나 해당 계좌는 현재 거래가 불가합니다."),
    NO_TRANSFER(BAD_REQUEST,"대기 중인 계좌 양도가 없습니다"),
    INVALID_TRANSFER(BAD_REQUEST, "유효하지 않은 양도 요청입니다."),

    // 거래내역 접근 요청
    NOT_EXIST_ACCESS_REQUEST(BAD_REQUEST, "존재하지 않거나 유효하지 않은 요청입니다."),
    EMPTY_ACCESS_REQUEST(BAD_REQUEST, "요청내역이 존재하지 않습니다"),
    ALREADY_EXISTED_ACCESS(BAD_REQUEST, "이미 허용되었거나 현재 요청 중입니다."),
    NO_ACCESSIBLE_PET_ACCOUNT(BAD_REQUEST, "접근이 허용된 펫계좌가 존재하지 않습니다."),

    // 거래내역 접근 가능 여부
    NO_PERMISSION_TO_TRANSACTION(BAD_REQUEST, "해당 계좌의 거래내역에 접근할 수 없습니다."),

    // 결제
    NO_PAYMENT(BAD_REQUEST, "해당하는 결제 요청이 존재하지 않습니다."),
    ...
    NO_TRANSACTION_DATA(BAD_REQUEST, "해당 거래내역의 정보가 존재하지 않습니다."),

    //파일
    S3_DELETE_FAILURE(BAD_REQUEST,"삭제할 파일이 존재하지 않습니다.");
