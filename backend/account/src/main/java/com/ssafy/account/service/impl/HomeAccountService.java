...
public class HomeAccountService {
    ...
    public List<HomeAccountResponse> getHomeAccountDetail(Long memberId) {
        List<PetAccount> normalPetAccounts = accountRepository.findAccountsByMemberIdAndAccountType(memberId, "00");

        if(normalPetAccounts.isEmpty()) {
        ...
        for (PetAccount normalPetAccount : normalPetAccounts) {

            List<Transaction> transactions = transactionRepository.findTop5ByAccountOrderByTransactionTimeDesc(normalPetAccount);
            ...
            result.add(new HomeAccountResponse(normalPetAccount, top5Transactions));
        }
